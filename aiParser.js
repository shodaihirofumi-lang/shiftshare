...
- 【最重要・行ズレ注意】各行では「日付セル」と、その勤務内容セルが左右に隣り合っています。1日ずつ、日付とすぐ隣の内容を必ずセットで読み取ってください。
- 期間の最初の週は、左列の上部にグレーの空セルがあり、日付が週の途中（例:水曜）から始まることがよくあります。空セルの分だけ下にずれるので、上から順に数えず、各セルに実際に書かれた日付の数字を見て対応させてください（グレーの空セルは日付として数えない）。
- 隣接する日の内容につられないでください。例えば「6:15-20:30」の隣の日が「希望休」でも、勤務日は勤務日として正しく読み取ってください。
- シフト内容は次のいずれかです:
